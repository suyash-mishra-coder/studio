...
export const mockSessions: InterviewSession[] = [
  { id: 'session-1', name: "Alex Doe", role: 'Frontend Engineer', specialty: 'Frontend Development', date: today.toISOString(), score: 8 },
  { id: 'session-2', name: "Alex Doe", role: 'Backend Engineer', specialty: 'Backend Development', date: yesterday.toISOString(), score: 6 },
  { id: 'session-3', name: "Alex Doe", role: 'Data Scientist', specialty: 'Data Science', date: twoDaysAgo.toISOString(), score: 9 },
];
...
export const mockFeedbacks: Record<string, InterviewFeedback> = {
  'session-1': {
    ...
    communicationAnalysis: 'You speak clearly and confidently. Your pacing is good, but you could pause more often to structure your thoughts on complex questions.',
  ...
  'session-2': {
    ...
    communicationAnalysis: 'Your explanations are technically sound, but can be a bit rushed. Try to slow down and ensure the interviewer is following along.',
  ...
  'session-3': {
    ...
    communicationAnalysis: 'Very articulate and structured answers. You do a great job of breaking down complex topics into understandable pieces.',
...
export const getSessions = async (): Promise<InterviewSession[]> => {
  ...
  const savedSessions = typeof window !== 'undefined' ? localStorage.getItem('mockview-sessions') : null;
...
export const getFeedback = async (sessionId: string): Promise<InterviewFeedback | undefined> => {
  ...
  const savedFeedbacks = typeof window !== 'undefined' ? localStorage.getItem('mockview-feedbacks') : null;
  if (savedFeedbacks) {
